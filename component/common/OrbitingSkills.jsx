...
const OrbitingSkill = memo(({ config, angle }) => {
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full p-2 bg-gray-900/90 backdrop-blur-md rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer border border-white/10 ${isHovered ? "scale-125 border-white/30" : "hover:border-white/20"}`}
        style={{
          boxShadow: isHovered
...
export default function OrbitingSkills() {
  ...
  // Scaled radii so icons never get cropped/covered on mobile
  const innerRadius = isMobile ? 65 : 95;
  const outerRadius = isMobile ? 120 : 175;
  const innerIconSize = isMobile ? 32 : 40;
  const outerIconSize = isMobile ? 38 : 46;

  const dynamicSkillsConfig = [
    {
      id: "html",
  ...
  return (
    <div className="w-full flex items-center justify-center py-4 sm:py-6">
      ...
      >
        {/* Center Tech Core */}
        <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-gray-800 to-black rounded-full flex items-center justify-center z-10 relative border border-white/10 shadow-2xl">
          <div className="absolute inset-0 rounded-full bg-cyan-500/15 blur-lg animate-pulse" />
          <div
            className="absolute inset-0 rounded-full bg-purple-500/15 blur-xl animate-pulse"
            style={{ animationDelay: "1.5s" }}
          />
          <div className="relative z-10 opacity-85">
            <svg
              width="20"
              height="20"
        ...
        {/* Orbit Rings */}
        <GlowingOrbitPath radius={innerRadius} glowColor="blue" />
        <GlowingOrbitPath
          radius={outerRadius}
          glowColor="green"
          animationDelay={2}
