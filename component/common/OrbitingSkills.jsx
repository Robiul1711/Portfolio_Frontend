...
// Updated Glow Colors to match standard Brand Identity
const glowColors = {
  cyan: {
...
const skillsConfig = [
  { id: "html", orbitRadius: 100, size: 40, speed: 0.8, iconType: "html", phaseShift: 0, label: "HTML5" },
  { id: "css", orbitRadius: 100, size: 40, speed: 0.8, iconType: "css", phaseShift: (2 * Math.PI) / 3, label: "CSS3" },
  { id: "javascript", orbitRadius: 100, size: 40, speed: 0.8, iconType: "javascript", phaseShift: (4 * Math.PI) / 3, label: "JavaScript" },
  { id: "react", orbitRadius: 180, size: 50, speed: -0.5, iconType: "react", phaseShift: 0, label: "React" },
  { id: "node", orbitRadius: 180, size: 45, speed: -0.5, iconType: "node", phaseShift: (2 * Math.PI) / 3, label: "Node.js" },
  { id: "tailwind", orbitRadius: 180, size: 40, speed: -0.5, iconType: "tailwind", phaseShift: (4 * Math.PI) / 3, label: "Tailwind CSS" },
];
...
const OrbitingSkill = memo(({ config, angle }) => {
  ...
  const iconColor = iconComponents[iconType]?.color || "#06B6D4";
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full p-2 bg-gray-900/80 backdrop-blur-md rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer border border-white/5 ${isHovered ? "scale-125 border-white/20" : "hover:border-white/10"}`}
        style={{
          boxShadow: isHovered
      ...
      >
        ...
        {isHovered && (
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-gray-900/95 backdrop-blur-sm rounded-lg text-[10px] font-bold text-white border border-white/10 whitespace-nowrap pointer-events-none shadow-xl">
            {label}
          </div>
        )}
      </div>
...
export default function OrbitingSkills() {
  ...
  return (
    <main className="w-full flex items-center justify-center overflow-hidden">
      ...
      >
        {/* Center Tech Core */}
        <div className="w-16 h-16 bg-gradient-to-br from-gray-800 to-black rounded-full flex items-center justify-center z-10 relative border border-white/10 shadow-2xl">
          <div className="absolute inset-0 rounded-full bg-cyan-500/10 blur-xl animate-pulse" />
          <div className="absolute inset-0 rounded-full bg-purple-500/10 blur-2xl animate-pulse" style={{ animationDelay: "1.5s" }} />
          <div className="relative z-10 opacity-80">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="url(#core-grad)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <defs>
        ...
        {/* Orbit Rings */}
        <GlowingOrbitPath radius={100} glowColor="blue" />
        <GlowingOrbitPath radius={180} glowColor="green" animationDelay={2} />
